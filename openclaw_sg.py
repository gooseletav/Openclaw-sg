...
conn.commit()
conn.close()
return {"is_new": True, "price_dropped": False, "drop_pct": 0, "repost_count": ph = json.loads(row["price_history"] or "[]")
...
rc += 1
cur.execute(
"UPDATE listings SET last_seen=?,rent_price=?,price_history=?,repost_count=? WHERE li
...
log.error("Craigslist error %s: %s", url, ex)
log.info("Craigslist: %d listings", len(results))
return results
def collect_apify():
if not APIFY_TOKEN:
return []
log.warning("No APIFY_TOKEN — skipping Zillow/Apartments/Zumper/Realtor.")
try:
...
hood = item.get("neighborhood") or item.get("city") or (addr.split(",")[-2].stri
src_map = {"zillow": "zillow", "realtor": "realtor.com", "zumper": "zumper", "apa
src lid = src_map.get(item.get("provider", ""), item.get("provider", "apify"))
= hashlib.md5((url or title + str(price)).encode()).hexdigest()
results.append({
"listing_id": lid,
...
SOURCE_EMOJI = {
"zillow": "Zillow", "apartments.com": "Apartments",
"zumper": "Zumper", "realtor.com": "Realtor", "craigslist": "Craigslist",
}
...
stars = "S" * min(5, max(1, int(score / 20)))
src = SOURCE_EMOJI.get(listing.get("source", ""), listing.get("source", "").upper())
kw_line = ""
...
dom_line = ("\nOn market: " + str(details["dom"]) + " days") if details["dom"] > 0 savings = int(listing["rent_price"] - offer)
beds = listing.get("bedrooms", 0)
return (
"OpenClaw-SG [" + src + "]\n\n"
+ listing["title"] + "\n"
+ listing["neighborhood"] + " - " + str(int(beds)) + "BR\n"
